        ...
        foreach ($params as $type => $paramArray) {
            ...
            foreach ($paramArray as $param) {
                if (strtolower($type) === "string") {
                    ...
                    if (is_object($param)) {
                        $className = SL::Services()->formatService->getClassName($param);
                        if((($className === "PlayerExtendedViewModelPublic") || ($className === "PlayerViewModelPublic") || ($className === "PlayerViewModelTokenizedPublic")) && (strtolower($type) === 'player')){
                            return true;
                        }
                        if(((($className === "GameView") || $className === "GameViewModelSmallest") || ($className === "Game")) && (strtolower($type) === 'game')){
                            return true;
                        }
                        if (strtolower($className) === strtolower($type)) {
                            return true;
                        }
                        MessageService::getInstance()->add('error', "[$callingMethod] - Expected [object " . $type . "]. Got [" . $className . "]");
                    }
                }
                return false;
            }
