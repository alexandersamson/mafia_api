...
namespace Main\Connection;

require_once($_SERVER["DOCUMENT_ROOT"] . '/Services/CheckValidAccess.php');
...
class Connection
{
    ...
    private function __construct(){
        self::$objectService = ObjectService::getInstance();
        $this->connect();
    }
    ...
    public function connect(){
        try {
            self::$connection = new PDO('mysql:host='.GlobalsService::$dbServername.';dbname='.GlobalsService::$dbDatabase, GlobalsService::$dbUsername, GlobalsService::$dbPassword);
    ...
    public function insertObjectIntoTable($object){
        ...
        $table = self::$objectService->fromCamelCase(get_class($object)).'s';
        $data = self::$objectService->prepareObjectForDbase($object);
        $fieldString = '(';
        $placeholderString = '(';
        $preparedData = [];
        ...
        $pStatement = self::$connection->prepare($sql);
    ...
    public function runMigrations($migrations){
        ...
        foreach ($migrations as $key => $value){
            ...
            if($result->fetchColumn() == 0){
                try {
                    $result = self::$connection->prepare($value);
                    $result->execute();
                    try {
